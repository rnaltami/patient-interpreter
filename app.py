...
if authentication_status:
    ...
    if page == "🩺 Patient Interpreter":
        ...
        st.text_area(
            label="",
            placeholder="What do you want to talk to your doctor about? Not sure yet? Symptoms Chat can help you figure it out.",
            key="symptoms_text",
            height=200
        )
        ...
        if st.session_state.rewritten and st.session_state.questions:
            if st.button("💾 Save This Visit Note"):
                ...
                filename = f"visit_logs/{username}_visit_log.csv"
    ...
    # ---------------------
        ...
        filename = f"visit_logs/{username}_visit_log.csv"
        summary_path = f"visit_summary/{username}_summary.csv"
            ...
            for i, row in df.iterrows():
                ...
                with st.expander(f"🕒 {date} – \"{snippet_preview}\""):
                    ...
                    if st.button("💾 Save Note", key=f"save_note_{i}"):
                        ...
                        df.at[i, "visit_date"] = visit_date.strftime("%Y-%m-%d") if visit_date else ""
